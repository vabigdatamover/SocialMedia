...
class MyTrendReq(TrendReq):
    ...
    def interest_by_region(self, resolution='CITY', inc_low_vol=False,
                           inc_geo_code=False):
...
#     pytrend = MyTrendReq()
#     pytrend.build_payload(kw_list=['Donald Trump', 'Joe Biden'],geo='US',timeframe='2020-03-01 2020-11-02')
#     #region_df = pytrend.interest_by_region(resolution='REGION', inc_low_vol=True, inc_geo_code=True)    
#     city_df = pytrend.interest_by_region(resolution='CITY', inc_low_vol=True, inc_geo_code=True)
#     dma_df = pytrend.interest_by_region(resolution='DMA')

if __name__=="__main__":
    pytrend = MyTrendReq()
    kw_list=['Donald Trump', 'Joe Biden']
    pytrend.build_payload(kw_list,geo='US',timeframe='2020-03-01 2020-11-02')
    city_df = pytrend.interest_by_region(resolution='CITY', inc_low_vol=True, inc_geo_code=True)
